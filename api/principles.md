...
> ⚠️ **建议使用 USB 扩展坞**（例如 CH334 USB 2.0 Hub）：PIO-USB 是通过 RP2350 的 PIO 状态机软件模拟的 USB 控制器，而非硬件 USB PHY。直接连接键盘或鼠标时，部分设备在热插拔时可能出现枚举失败，需要重新上电才能识别。使用扩展坞可以有效缓解此问题——扩展坞自带硬件 USB PHY，电气信号更稳定，PIO-USB 只需与扩展坞通信即可。
...
- **Web UI**：PICO 配置 → 危险操作 → 恢复出厂设置。设备运行中删除全部 KV key 并擦除 Lua 脚本，随后自动重启。
