...
`dmo_` = direct mouse output（直接鼠标输出）。与上面的 `input_mouse_move` 不同：这一组**不经过映射引擎**，直接把位移、滚轮、按键作为一台 USB 鼠标发给手机/电脑——脚本给多少像素光标就走多少（不乘"鼠标速度"倍率）。

只在「光标显示方式 → 直接鼠标」或「目标平台 → 苹果」下可用（这两项都要**重启设备**后才生效）。未启用时这些函数不报错，只是什么都不做。

| 函数 | 参数 | 说明 |
|------|------|------|
| `dmo_move(dx, dy[, wheel])` | 相对位移（像素，正 dx = 右、正 dy = 下）；`wheel` 缺省 0，正 = 上滚 | 移动光标 / 滚动。纯滚动写 `dmo_move(0, 0, -1)`。 |
